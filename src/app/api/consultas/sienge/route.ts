...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Lista de inadimplentes (parcelas vencidas em aberto) p/ a cobrança curada.
    if (action === "inadimplentes") {
      const itens = await listarInadimplentes(new Date());
      return NextResponse.json({ ok: true, action, total: itens.length, itens });
    }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (acao === "enviar-lembretes") {
      const itens = Array.isArray(body.itens) ? body.itens : [];
      if (!itens.length) return NextResponse.json({ ok: false, error: "sem_itens" }, { status: 400 });
      const resultado = await enviarLembretes(itens.slice(0, 200), new Date());
      return NextResponse.json({ ok: true, action: acao, ...resultado });
    }
